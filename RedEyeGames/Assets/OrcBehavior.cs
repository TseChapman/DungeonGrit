...
public class OrcBehavior : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // if it is a Tracking enemy                // or its an ORC                   and has                   
        if (mEnemyBehavior == EnemyBehavior.TRACKING || mEnemyBehavior == EnemyBehavior.ORC &&      
            // greater then 25% health                                                                  OR     distance from player is greater then tracking range 
            (hero.GetComponent<Health>().GetHealth() > (hero.GetComponent<Health>().GetMaxHealth() / 4) ||  Vector3.Distance(attackPoint.transform.position, hero.transform.position) < mTrackingRange))
        {
            TrackHero(isGound);
        }

                                                        // or ORC with < 25% health and distance from player is satisfactory
        else if (mEnemyBehavior == EnemyBehavior.PATROL || mEnemyBehavior == EnemyBehavior.ORC)
        {
